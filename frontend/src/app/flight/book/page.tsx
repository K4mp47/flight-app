...
const SimplifiedSeatGrid = ({
  ...
  const seatField =
    flightType === "outbound" ? "id_seat_outbound" : "id_seat_return";
  ...
  const getSeatStatus = (
    ...
    if (
      seat.id_cell === currentPassengerSeat ||
      assignedSeats.includes(seat.id_cell)
    )
      return "selected";
    return "available";
  };
  ...
  const seatsByClass = seatBlocks.reduce(
    (acc, block) => {
    ...
    <div className="bg-card rounded-lg">
      ...
      <div className="p-3 sm:p-6">
        ...
        <div className="space-y-6 sm:space-y-8 overflow-x-auto">
          {Object.entries(seatsByClass)
            .sort(([a], [b]) => Number(a) - Number(b))
            .map(([classType, classSeats]) => {
              ...
              return (
                <div key={classType}>
                  <h4 className="text-xs sm:text-sm font-semibold text-gray-200 mb-3 sm:mb-4">
                    {getClassLabel(Number(classType))}
                  </h4>
                  ...
                  <div className="inline-flex min-w-full flex-col justify-center items-center">
                    ...
                    <div className="flex mb-2 ml-6 sm:ml-8">
                      {Array.from({ length: classCols }, (_, i) => (
                        <div
                          key={i}
                          className="w-10 sm:w-14 text-center text-xs font-medium text-gray-200"
                        >
                          {minX + i + 1}
                        </div>
                      ))}
                          ...
                          {Array.from({ length: classCols }, (_, x) => {
                            ...
                            return (
                              <button
                                key={x}
                                onClick={() =>
                                  !isDisabled && onSeatSelect(seat.id_cell)
                                }
                                disabled={isDisabled}
                                className={`
                                ...
                                <div className="relative">
                                  ...
                                  {status === "selected" && (
                                    <div className="absolute inset-0 flex items-center justify-center">
                                      <Check
                                        className="w-3 sm:w-5 h-3 sm:h-5 text-white mt-0.5 sm:mt-1"
                                        strokeWidth={3}
                                      />
                                    </div>
                                  )}
...
function FlightBookingContent() {
  ...
  const [expandedPassengerIndex, setExpandedPassengerIndex] = useState<
    number | null
  >(0);
  ...
  useEffect(() => {
    const fetchSeats = async () => {
      ...
      setLoading(true);
      try {
        const outboundData = await api.get<SeatBlock[]>(
          `/flight/${outboundFlightId}/seat-availability`
        );
        console.log("Selected flight class:", selectedFlightClass);
        console.log(
          "Available seat blocks:",
          outboundData.map(b => ({
            id_class: b.id_class,
        ...
        const filteredOutbound = outboundData.filter(
          block => block.id_class === selectedFlightClass
        );
        console.log(
          "Filtered blocks:",
          filteredOutbound.map(b => ({
            id_class: b.id_class,
            seats: b.seats.length,
          }))
        );
        ...
        if (returnFlightId) {
          const returnData = await api.get<SeatBlock[]>(
            `/flight/${returnFlightId}/seat-availability`
          );
          // Filter to only show seats for the selected class
          const filteredReturn = returnData.filter(
            block => block.id_class === selectedFlightClass
          );
  ...
  const handleSeatSelect = (seatId: number, type: "outbound" | "return") => {
    const seatField =
      type === "outbound" ? "id_seat_outbound" : "id_seat_return";
    const updated = [...passengers];

    // Check if seat is already assigned to another passenger
    const seatAlreadyAssigned = passengers.some(
      (p, idx) => idx !== selectedPassengerIndex && p[seatField] === seatId
    );
  ...
  const canProceedToInfo = () => {
    const allSeatsSelected = passengers.every(
      p => p.id_seat_outbound && (!returnFlightId || p.id_seat_return)
    );
  ...
  return (
    <div className="min-h-screen bg-background mt-20">
      <Button
        variant="outline"
        className="hidden lg:flex lg:fixed  left-2 sm:left-4 top-2 sm:top-4 z-50 border-0! p-2 sm:p-3 justify-center"
        onClick={() => router.back()}
      >
            ...
            {currentStep === "seats" ? (
              <div className="space-y-4 sm:space-y-6">
                <div>
                  ...
                  <p className="text-sm sm:text-base text-primary/20">
                  ...
                  <div className="flex gap-2 flex-wrap overflow-x-auto pb-2">
                    {passengers.map((p, index) => (
                      <button
                        ...
                        className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-md font-medium text-xs sm:text-sm transition-colors shrink-0 ${
                ...
                <div className="flex justify-end mb-2 sm:mb-0">
                  <div className="flex gap-2 md:mb-6">
                    <button
                      onClick={() =>
                        setSelectedPassengerIndex(prev =>
                          prev > 0 ? prev - 1 : prev
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        setSelectedPassengerIndex(prev =>
                          prev < passengers.length - 1 ? prev + 1 : prev
                        )
                      }
                      disabled={
                        selectedPassengerIndex === passengers.length - 1
                      }
                      className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-md font-medium text-xs sm:text-sm bg-card text-ring hover:bg-gray-400/20 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next Passenger
                  ...
                  returnSeatBlocks.length === 0 ? (
                    <div className="bg-card rounded-lg p-6 text-center">
                      <p className="text-destructive font-medium">
                        No seats available for the selected flight(s)
                      </p>
                      <Button
                        variant="outline"
              ...
              <div className="space-y-4 sm:space-y-6">
                <div>
                  ...
                  <p className="text-sm sm:text-base text-primary">
                    Complete details for all passengers
                  </p>
                </div>
                ...
                <div className="space-y-2 sm:space-y-4">
                  {passengers.map((passenger, index) => (
                    <Card
                      key={index}
                      className={`border-0 ${expandedPassengerIndex === index ? "" : "hover:opacity-80"}`}
                      onClick={() =>
                        setExpandedPassengerIndex(
                          expandedPassengerIndex === index ? null : index
                        )
                      }
                    >
                            ...
                            <div className="space-y-1 sm:space-y-2">
                              ...
                              <Select
                                ...
                                onValueChange={(value: "M" | "F") =>
                                  updatePassenger(index, "sex", value)
                                }
                              >
                                <SelectTrigger
                                  id={`gender-${index}`}
      ...
      <div className="lg:hidden fixed bottom-4 right-4 z-50">
        <Sheet open={showMobileSummary} onOpenChange={setShowMobileSummary}>
          <SheetTrigger asChild>
            <Button
              size="lg"
              className="shadow-lg rounded-full h-14 w-14 sm:h-16 sm:w-16"
            >
          ...
          <SheetContent
            side="right"
            className="w-full sm:w-100 p-0 overflow-y-auto"
          >
