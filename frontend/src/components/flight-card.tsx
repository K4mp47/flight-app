...
export function FlightCard({ flight, className }: FlightCardProps) {
  ...
  const handleSelectFlight = () => {
    if (flight.id_flight) {
      // Assuming navigation to a booking page that takes flight ID as a param
      router.push(`/flight/book?outboundFlightId=${flight.id_flight}&baseFlightPrice=${flight.base_price}`);
    } else {
    ...
    <Card className={cn("w-full max-w-4xl mx-auto transition-all hover:shadow-lg hover:-translate-y-1", className)}>
      <CardContent className="p-4 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        ...
        <div className="md:col-span-3 flex flex-col items-end justify-center gap-2 text-right">
          ...
          <Button className="w-full mt-2 md:w-auto" onClick={handleSelectFlight}>Select Flight</Button>
        </div>
